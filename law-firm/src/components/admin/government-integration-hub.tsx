...
const statusMeta: Record<IntegrationStatus, { ar: string; en: string; className: string }> = {
  available: { ar: "متاح برمجياً", en: "Programmatic access", className: "border-emerald-400/25 bg-emerald-400/10 text-emerald-200" },
  "official-link": { ar: "رابط حكومي رسمي", en: "Official government link", className: "border-sky-400/25 bg-sky-400/10 text-sky-200" },
  "requires-onboarding": { ar: "يتطلب اعتماد/تسجيل", en: "Onboarding required", className: "border-amber-300/25 bg-amber-300/10 text-amber-100" },
  research: { ar: "قيد التحقق", en: "Research only", className: "border-white/15 bg-white/5 text-white/60" },
};
    ...
    <header className="border border-white/10 bg-[#102a31] p-5 sm:p-7">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
        ...
        <div className="grid min-w-0 grid-cols-2 gap-2 sm:min-w-80">
          <div className="border border-emerald-400/20 bg-emerald-400/5 p-4"><CheckCircle2 className="mb-3 text-emerald-300" size={20}/><strong className="block text-2xl">{counts.verified}</strong><span className="text-xs text-white/45">{ar ? "مصادر وصول مباشرة" : "direct-access sources"}</span></div>
          <div className="border border-amber-300/20 bg-amber-300/5 p-4"><Wrench className="mb-3 text-amber-200" size={20}/><strong className="block text-2xl">{counts.onboarding}</strong><span className="text-xs text-white/45">{ar ? "تحتاج تسجيل/اعتماد" : "need onboarding"}</span></div>
        </div>
      </div>
    </header>

    <div className="border border-amber-300/20 bg-amber-300/5 p-4 text-sm leading-7 text-amber-50/80">
      <div className="flex items-start gap-3"><ShieldAlert className="mt-1 shrink-0 text-amber-200" size={19}/><p>{ar ? "قاعدة أمان: لا ترسل أرقاماً شخصية أو ملفات قضايا أو مستندات سرية إلى أي API أو نموذج ذكاء اصطناعي لم تتم الموافقة عليه تعاقدياً وتقنياً. الروابط الحكومية هنا لا تعني وجود صلاحية لقراءة بيانات العميل تلقائياً." : "Security rule: never send CPRs, case files or confidential documents to any API or AI model without contractual and technical approval. An official link does not imply permission to automatically read client data."}</p></div>
    </div>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item) => {
        ...
        return <article key={item.id} className="flex min-w-0 flex-col border border-white/10 bg-[#102a31] p-5 sm:p-6">
          ...
          <ul className="mt-4 grid gap-2 border-t border-white/8 pt-4">{points.map((point) => <li key={point} className="flex gap-2 text-xs leading-5 text-white/55"><CheckCircle2 className="mt-0.5 shrink-0 text-emerald-300" size={14}/><span>{point}</span></li>)}</ul>
