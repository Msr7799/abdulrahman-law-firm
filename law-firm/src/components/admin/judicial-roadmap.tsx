        ...
        <aside className="border-b border-white/10 bg-black/15 p-3 lg:border-b-0 lg:border-e">
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
            {judicialRoadmaps.map((item, index) => {
              ...
              return <LiquidButton key={item.id} type="button" onClick={() => setSelected(item.id)} className={`roadmap-nav-item focus-ring group flex min-h-20 items-center gap-3 border p-3 text-start transition ${active ? "roadmap-nav-active border-[#b89555]/50 bg-[#b89555]/12 text-white" : "border-transparent text-white/50 hover:border-white/10 hover:bg-white/[.035] hover:text-white"}`}><span className={`roadmap-nav-index grid size-9 shrink-0 place-items-center text-xs font-black ${active ? "bg-[#b89555] text-[#0c1c21]" : "bg-white/5 text-white/35"}`}>{String(index + 1).padStart(2, "0")}</span><span className="min-w-0"><strong className="roadmap-nav-title block text-sm">{ar ? item.titleAr : item.titleEn}</strong><small className="roadmap-nav-audience mt-1 block truncate text-[10px] opacity-55">{ar ? item.audienceAr : item.audienceEn}</small></span><ChevronDown className={`roadmap-nav-arrow ms-auto shrink-0 transition lg:-rotate-90 ${active ? "text-[#d0ad69]" : "opacity-0 group-hover:opacity-50"}`} size={15} /></LiquidButton>;
            })}
          </div>
        </aside>
