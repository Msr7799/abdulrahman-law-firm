...
export function BookingForm({ locale }: { locale: Locale }) {
  ...
  const consultationType = useWatch({ control, name: "type" });
    ...
    >
      ...
      <div className="grid gap-6 p-6 sm:grid-cols-2 sm:p-10">
        ...
        <div className="text-sm font-bold">
          <span className="mb-1 block">{ar ? "نوع الاستشارة" : "Consultation type"}</span>
          <input type="hidden" {...register("type")} />
          <SelectDropdown
            value={consultationType ?? ""}
            onValueChange={(value) => setValue("type", value as ConsultationInput["type"], { shouldValidate: true, shouldDirty: true })}
            ariaLabel={ar ? "نوع الاستشارة" : "Consultation type"}
            placeholder="—"
            options={[
              { value: "office", label: ar ? "في المكتب" : "In office" },
              { value: "phone", label: ar ? "هاتفية" : "By phone" },
              { value: "remote", label: ar ? "عن بُعد" : "Remote" },
            ]}
            className={`${input} justify-between text-start`}
            contentClassName="booking-select-menu"
          />
          {fieldError("type")}
        </div>
