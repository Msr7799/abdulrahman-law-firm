  ...
  return (
    <section className="grid min-h-[680px] overflow-hidden border border-white/10 lg:grid-cols-[1fr_280px]">
      <div className="flex min-h-[680px] flex-col bg-[#0c1c21]">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 p-5"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center bg-[#b89555]/15 text-[#d0ad69]"><Bot /></span><div><h2 className="font-bold">{ar ? "وكيل القضايا والقانون البحريني" : "Bahrain cases & law agent"}</h2><p className="mt-1 text-xs text-white/40">RAG · Tavily · Gemini</p></div></div><button onClick={() => { setMessages((current) => current.slice(0, 1)); setError(""); }} className="focus-ring flex items-center gap-2 p-2 text-xs text-white/45 hover:text-white"><Trash2 size={15} />{ar ? "محادثة جديدة" : "New chat"}</button></div>
