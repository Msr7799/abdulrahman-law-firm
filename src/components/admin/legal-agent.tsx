          ...
          <div className={`legal-agent-messages h-full min-h-0 min-w-0 overflow-x-hidden overflow-y-auto overscroll-contain px-3 [-webkit-overflow-scrolling:touch] [scrollbar-gutter:stable] sm:px-6 lg:px-8 ${messages.length <= 1 ? "flex flex-col" : "space-y-5 py-5"}`}>
            ...
            {messages.filter((message) => message.id !== "welcome").map((message) => {
              ...
              >
                ...
                {answerSettled && message.nodes && <div className="mt-5 flex flex-wrap gap-2 border-t border-white/8 pt-4">{message.nodes.map((node) => <span key={node.id} className={`flex items-center gap-1.5 border px-2.5 py-1 text-[10px] ${node.status === "done" ? "border-emerald-600/40 bg-emerald-100 text-emerald-800" : node.status === "error" ? "border-red-400/20 bg-red-400/10 text-red-700" : "border-white/10 text-white/35"}`}>{node.status === "done" ? <CheckCircle2 size={12} /> : <CircleDashed size={12} />}{node.label}{node.ms > 0 && ` · ${node.ms}ms`}</span>)}</div>}
