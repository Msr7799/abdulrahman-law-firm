    ...
    <section className="border border-white/10 bg-[#0c1c21]">
      ...
      {current ? <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_15rem_auto] lg:items-center">
        ...
        <div className="admin-news-image relative min-h-36 overflow-hidden rounded-md border border-white/10 bg-[#10191b] shadow-sm">
          {current.imageUrl ? <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(to top,rgba(8,8,8,.35),rgba(8,8,8,.02)),url(${JSON.stringify(current.imageUrl).slice(1,-1)})` }} /> : current.sourceLogoUrl || current.relatedLogos?.length ? <NewsLogoCluster item={current} mode="compact" /> : <div className="absolute inset-0 grid place-items-center"><Newspaper className="text-[#d0ad69]/45" size={42} /></div>}
