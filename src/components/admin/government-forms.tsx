...
export function GovernmentForms({ locale }: { locale: Locale }) {
  ...
  return <section className="grid gap-7">
    <header className="border border-white/10 bg-[#102a31] p-5 sm:p-7"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
      ...
      <div className="admin-official-source flex items-center gap-3 border border-emerald-300/20 bg-emerald-300/5 p-4 text-xs leading-6 text-emerald-100/75"><ShieldCheck className="shrink-0 text-emerald-300" size={19} />{ar ? "الروابط والشعارات من صفحة خدمات وزارة العدل الرسمية." : "Links and service marks are from the official Ministry of Justice services page."}</div>
    </div></header>
    ...
    <section className="no-print border-t border-white/10 pt-7"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
      ...
      {groupedForms.length ? <Accordion multiple value={openCategories} onValueChange={(value) => setOpenCategories(value.filter((item): item is string => typeof item === "string"))} className="mt-4 grid gap-3">{groupedForms.map((group) => {
        ...
        return <AccordionItem key={group.id} value={group.id} className={`overflow-hidden border ${open ? "border-[#771111]/70 bg-[#771111]/[.045]" : "border-white/10 bg-white/[.025]"}`}>
          <AccordionTrigger className="focus-ring min-h-18 w-full items-center px-5 py-4 text-start hover:no-underline"><span className="flex min-w-0 items-center gap-3"><span className={`admin-form-category-icon grid size-10 shrink-0 place-items-center ${open ? "bg-[#771111] text-white" : "bg-white/[.05] text-[#d1b579]"}`}>{open ? <FolderOpen size={19} /> : <Folder size={19} />}</span><span className="min-w-0"><strong className="block text-sm text-white/90">{ar ? meta.ar : meta.en}</strong><span className="mt-1 block text-[11px] font-normal text-white/35">{ar ? `${group.forms.length} استمارة داخل القسم` : `${group.forms.length} forms in this section`}</span></span></span></AccordionTrigger>
