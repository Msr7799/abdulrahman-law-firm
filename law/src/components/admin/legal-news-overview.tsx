  ...
  return (
    <section className="border border-white/10 bg-[#0c1c21]">
      ...
      <div className="grid gap-px bg-white/10 sm:grid-cols-3">
        <div className="bg-[#0c1c21] p-4"><span className="text-[10px] text-white/35">{ar ? "أخبار هذا الأسبوع" : "This week"}</span><strong className="admin-news-stat mt-1 block text-2xl text-[#ead39f]">{loading ? "…" : items.length}</strong></div>
        <div className="bg-[#0c1c21] p-4"><span className="text-[10px] text-white/35">{ar ? "تشريعات" : "Legislation"}</span><strong className="admin-news-stat mt-1 block text-2xl text-[#ead39f]">{loading ? "…" : legislationCount}</strong></div>
        <div className="bg-[#0c1c21] p-4"><span className="text-[10px] text-white/35">{ar ? "مصادر رسمية/حكومية" : "Official/government"}</span><strong className="admin-news-stat mt-1 block text-2xl text-[#ead39f]">{loading ? "…" : officialCount}</strong></div>
      </div>
      {current ? <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_15rem_auto] lg:items-center">
        <div>
        ...
        <div className="admin-news-image relative min-h-36 overflow-hidden rounded-md border border-white/10 bg-[#10191b] shadow-sm">
          {current.imageUrl ? <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(to top,rgba(8,8,8,.35),rgba(8,8,8,.02)),url(${JSON.stringify(current.imageUrl).slice(1,-1)})` }} /> : current.sourceLogoUrl ? <div className="absolute inset-0 grid place-items-center bg-[#fffdf8] p-5"><img src={current.sourceLogoUrl} alt={current.sourceLogoName || current.sourceName} className="max-h-24 max-w-[82%] object-contain" /></div> : <div className="absolute inset-0 grid place-items-center"><Newspaper className="text-[#d0ad69]/45" size={42} /></div>}
          {current.imageUrl && current.sourceLogoUrl && <div className="absolute start-2 top-2 grid min-h-10 min-w-14 place-items-center rounded-sm border border-white/70 bg-white/95 px-2 py-1 shadow"><img src={current.sourceLogoUrl} alt={current.sourceLogoName || current.sourceName} className="max-h-7 max-w-16 object-contain" /></div>}
        </div>
