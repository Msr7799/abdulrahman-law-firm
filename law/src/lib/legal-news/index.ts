...
import { findBahrainLogoForText } from "@/lib/bahrain-logo-directory";
...
async function enrichNewsMedia(items: LegalNewsItem[]) {
  return Promise.all(items.map(async (item) => {
    ...
    const logo = await findBahrainLogoForText(`${item.sourceName} ${item.title} ${item.summary}`, item.sourceUrl);
    return {
      ...item,
      imageUrl: imageUrl || undefined,
      sourceLogoUrl: logo?.url,
      sourceLogoName: logo?.name,
    };
  }));
}
...
const getCachedLegalNews = unstable_cache(loadLegalNewsUncached, ["bahrain-legal-news-v3-logos"], {
